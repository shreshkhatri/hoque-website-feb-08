...
import { ArrowLeft, Save, X, User, Building, Loader2 } from 'lucide-react'
...
export default function EditTestimonialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchTestimonial = async () => {
      try {
        const res = await fetch(`/api/admin/testimonials/${id}`, { credentials: 'same-origin' })
        if (!res.ok) throw new Error('Failed to fetch')
        const data = await res.json()
        const t: Testimonial = data.data
        setForm({
          name: t.name,
          country: t.country,
          ...
          university_logo_url: t.university_logo_url || '',
          rating: String(t.rating),
          review: t.review,
          display_at_homepage: t.display_at_homepage,
          display_order: String(t.display_order),
          is_active: t.is_active,
        })
        if (t.photo_url) setPhotoPreview(t.photo_url)
        if (t.university_logo_url) setLogoPreview(t.university_logo_url)
      } catch (err) {
  ...
  const removeLogo = () => {
    setLogoFile(null)
    setLogoPreview(null)
    setForm((prev) => ({ ...prev, university_logo_url: '' }))
  ...
  const uploadLogo = async (): Promise<string | null> => {
    if (!logoFile) return form.university_logo_url || null
    ...
    try {
      ...
      return form.university_logo_url || null
    } finally {
